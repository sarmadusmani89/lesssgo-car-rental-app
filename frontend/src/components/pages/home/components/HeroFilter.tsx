                    ...
                    >
                        ...
                        {activeDropdown === 'pickup' && (
                            <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-2xl shadow-xl max-h-[280px] overflow-y-auto z-50 animate-in fade-in slide-in-from-top-2 duration-200" onClick={(e) => e.stopPropagation()}>
                                <div className="p-[14px_20px] text-[15px] text-muted-foreground font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6" onClick={() => handleSelect('pickup', '')}>Select Pickup</div>
                                {dropdownOptions.locations.map((opt: string) => (
                                    <div
                                        key={opt}
                                        className={`p-[14px_20px] text-[15px] font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6 ${selections.pickup === opt ? 'bg-secondary text-primary font-bold' : 'text-muted-foreground'}`}
                ...
                <div className="flex flex-col gap-2">
                    ...
                    >
                        ...
                        {activeDropdown === 'return' && (
                            <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-2xl shadow-xl max-h-[280px] overflow-y-auto z-50 animate-in fade-in slide-in-from-top-2 duration-200" onClick={(e) => e.stopPropagation()}>
                                <div className="p-[14px_20px] text-[15px] text-muted-foreground font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6" onClick={() => handleSelect('return', '')}>Select Return</div>
                                {dropdownOptions.locations.map((opt: string) => (
                                    <div
                                        key={opt}
                                        className={`p-[14px_20px] text-[15px] font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6 ${selections.return === opt ? 'bg-secondary text-primary font-bold' : 'text-muted-foreground'}`}
                ...
                <div className="flex flex-col gap-2">
                    ...
                    >
                        ...
                        {activeDropdown === 'category' && (
                            <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-2xl shadow-xl max-h-[280px] overflow-y-auto z-50 animate-in fade-in slide-in-from-top-2 duration-200" onClick={(e) => e.stopPropagation()}>
                                <div className="p-[14px_20px] text-[15px] text-muted-foreground font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6" onClick={() => handleSelect('category', '')}>Any Category</div>
                                {dropdownOptions.category.map((opt: string) => (
                                    <div
                                        key={opt}
                                        className={`p-[14px_20px] text-[15px] font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6 ${selections.category === opt ? 'bg-secondary text-primary font-bold' : 'text-muted-foreground'}`}
                    ...
                    >
                        ...
                        {activeDropdown === 'transmission' && (
                            <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-2xl shadow-xl max-h-[280px] overflow-y-auto z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                <div className="p-[14px_20px] text-[15px] text-muted-foreground font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6" onClick={() => handleSelect('transmission', '')}>Any</div>
                                {dropdownOptions.transmission.map((opt: string) => (
                                    <div
                                        key={opt}
                                        className={`p-[14px_20px] text-[15px] font-medium cursor-pointer transition-all duration-200 hover:bg-secondary/50 hover:text-accent hover:pl-6 ${selections.transmission === opt ? 'bg-secondary text-primary font-bold' : 'text-muted-foreground'}`}
                                        onClick={(e) => {
                                            e.stopPropagation();
