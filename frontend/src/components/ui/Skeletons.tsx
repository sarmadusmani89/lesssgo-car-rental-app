    ...
    return (
        <div className="w-full space-y-4 animate-pulse">
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="h-10 w-32 bg-muted/80 rounded-xl"></div>
            </div>

            <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
                <div className="h-12 bg-muted/30 border-b border-border"></div>
                <div className="divide-y divide-border">
                    {Array.from({ length: rows }).map((_, i) => (
                        <div key={i} className="flex gap-4 px-6 py-5">
                            {Array.from({ length: cols }).map((_, j) => (
                                <div key={j} className={`h-4 bg-muted/40 rounded flex-1 ${j === cols - 1 ? 'max-w-[100px] ml-auto' : ''}`}></div>
                            ))}
                        </div>
                    ))}
                </div>
            ...
            {Array.from({ length: cards }).map((_, i) => (
                <div key={i} className="bg-card p-6 rounded-2xl border border-border shadow-sm">
                    ...
                    <div className="flex justify-between items-center">
                        <div className="h-6 w-20 bg-muted/80 rounded-lg"></div>
